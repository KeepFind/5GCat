/*
	...
	daterange：判断日期范围
	idcard：对身份证号码进行严格验证
    name: 判断名称是否为中文、字母、数字
    money: 判断金额是否在11位内，可带两位小数
*/
...
(function () {
    if ($.Datatype) {
        $.extend($.Tipmsg.w, {
            ...
            "name": "请输入名称！",
            "money": "请输入金额！",
            "wrogeful": "文本框中不能含有 1 单引号: ' 2  竖 杠: |  3 尖角号: < > 请检查输入！",
            "postcode": "请输入邮编！",
            "donsonmoney":"请输入金额！"
        });
        ...
        $.extend($.Datatype, {
			...
			*/
            ...
            //注册账户：5-30位字母、数字、下划线、@以及.号组成
            "accounts": /^[a-zA-Z0-9_@.]{5,30}$/,

            //名称：1以上位中文、字母、数字多个空格、_-组成
            "name": /^[a-zA-Z0-9_-\s\u4e00-\u9fa5]{1,}$/,

            //金额：10位数之间的整数和两位小数组成
            "money": /^(([0-9]|([1-9][0-9]{0,9}))((\.[0-9]{1,2})?))$/,

            //文本框中不能含有 1 单引号: ' 2  竖 杠: |  3 尖角号: < > 请检查输入！
            "wrogeful": /^[^\|'<>]{1,}$/,

            // 邮编：国内邮编
            "postcode": /^[1-9]\d{5}(?!\d)$/,
            //验证带逗号的金额
            "donsonmoney": /^([1-9]\d{0,2}(,)?(,\d\d\d){1,3}(\.\d{1,2})?|[1-9]\d{1,11}(\.\d{1,2})?|^\d(\.\d{1,2})?)$/,
            /*
			  参数gets是获取到的表单元素值，
			  obj为当前表单元素，
			  curform为当前验证的表单，
